...
plot_triggering[[1]] <- triggering_fun_plot_prior(list.input, magnitude = 4, n.samp = 1000) +
  ggtitle("G. M4 trig. func.") +    
  xlab("Time [days]")+
  theme_bw()  + 
  theme(plot.title = element_text(size = 9))

plot_triggering[[3]] <- triggering_fun_plot_prior(list.input, magnitude = 6.7, n.samp = 1000) +
...
```{r}
plot_omori <- list()
plot_omori[[1]] <- omori_plot_prior(list.input, n.samp = 1000) +
