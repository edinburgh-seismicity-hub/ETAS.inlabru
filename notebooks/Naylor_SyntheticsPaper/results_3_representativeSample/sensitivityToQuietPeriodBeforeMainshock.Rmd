---
title: "Sensitivity of posteriors to quiecent period: What makes a representative sample?"
author: "Naylor"
date: "2022-10-27"
...
```{r}
## Analysis specifying start and end times, using full catalogue
...
plots[[1]] <- ggplot(post.df, aes(x=x,y=y, group=id, color=factor(id), lty=factor(id))) + 
  ...
  ggtitle(paste("(b) Posteriors of catalogue with M6.7 event on day 500 and variable start date" )) +
...
grid_arrange_shared_legend(catPlot, plots[[1]], ncol=1, nrow=2, position='bottom')
```

```{r fig.height=3, fig.width=5}
grid_arrange_shared_legend(catPlot, plots_V2[[1]], ncol=1, nrow=2, position='bottom')
```
...
runtimes <- c()
numberEvents <- c()
for(i in 1:length(list.output.quietRuninScenario) ){
  runtimes[i] <-list.output.quietRuninScenario[[i]]$runtime
  numberEvents[i] <-  list.output.quietRuninScenario[[i]]$nEvents
}
