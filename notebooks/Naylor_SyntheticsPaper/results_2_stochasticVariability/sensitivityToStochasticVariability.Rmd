...
multiplot(mu_priorPlot,K_priorPlot,alpha_priorPlot,c_priorPlot,p_priorPlot,cols = 3)
```
...
plots[[i]] <- ggplot() + 
  ...
  ggtitle(paste("Quiet syntheic catalogue",i,", nEvents =", length(M6p7.ETAS.cat.list[[i]]$ts) )) +
  ylim(2,7) +
  theme_bw() +
  xlab("Time [days]") +
  ylab("Magnitude")
...
ggsave("Sipp_10M6p7Cats.png", marrangeGrob(plots, nrow=5, ncol=2))
```
...
ggsave("Sipp_10QuietCats.png", marrangeGrob(plots, nrow=5, ncol=2))
```
...
plots[[1]] <- ggplot(post.df, aes(x=x,y=y, group=id, color=factor(id), lty=factor(id))) + 
  ...
  ggtitle(paste("Impact of stochastic variability for 1000 day catalogue with no large events seeded") ) +
...
plots[[2]] <- ggplot(post.df, aes(x=x,y=y, group=id, color=factor(id), lty=factor(id))) + 
  ...
  geom_vline(aes(xintercept = value ), data=trueParas, color='black', linetype=2, label="True value") + 
  labs(color='Random catalogue number', linetype='Random catalogue number' ) +
  ggtitle(paste("Impact of stochastic variability for 1000 day catalogue with a M6.7 event seeded on day 500")) +
  xlab("ETAS Posteriors") +
  theme_bw() +
  theme(axis.text.x = element_text(angle = 45, hjust=1))+
  theme(legend.position='hidden')
...
ggsave("stochasticVariability_posteriors.png", marrangeGrob(plots, nrow=2, ncol=1))
