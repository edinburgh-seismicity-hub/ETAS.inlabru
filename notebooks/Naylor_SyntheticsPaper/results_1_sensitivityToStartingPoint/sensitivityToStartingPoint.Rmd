...
plt <- grid.arrange(plots[[1]], plots[[3]], ncol=1, nrow=2) 
...
plot_triggering[[4]] <- triggering_fun_plot(list.output.M6p7Scenario[[2]], magnitude = 6.7, n.samp = 100) +
  ggtitle("(f) M6.7 trig. func. for M6.7 baseline") +     
  theme_bw() + 
  ylim(0,1700) + 
  theme(plot.title = element_text(size = 8))
...
```{r}
plot_omori <- list()
plot_omori[[1]] <- omori_plot(list.output.quietScenario[[2]], n.samp = 100) +
  ggtitle("(a) Omori decay for quiet baseline") +     
  theme_bw()  + 
  ylim(0,1) + 
  theme(plot.title = element_text(size = 8))


plot_omori[[2]] <- omori_plot(list.output.M6p7Scenario[[2]], n.samp = 100) +
  ggtitle("(b) Omori decay for M6.7 baseline")+     
  theme_bw()  + 
  ylim(0,1) + 
  theme(plot.title = element_text(size = 8))
...
plt <- grid.arrange(plot_omori[[1]], plot_triggering[[1]], plot_triggering[[3]],plot_omori[[2]],plot_triggering[[2]], plot_triggering[[4]], ncol=3, nrow=2, top="Triggering  function variability", left="M6.7 baseline                  Quiecent baseline")
