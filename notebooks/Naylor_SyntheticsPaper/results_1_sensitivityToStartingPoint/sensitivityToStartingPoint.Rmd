...
for (i in 1:nRealisations){
  ...
  list.input <- create.input.list.temporal.noCatalogue('../../../ETAS.inlabru/data/user_input_synthetic_noCatalogue.txt')
...
for (i in 2:nRealisations){
  ...
  # Load a set of parameters that we will need to tweak for this application
  list.input <- createDeafultInputList.temporal.noCatalogue('user_input_synthetic.txt')
...
```{r}
list.output.M6p7Scenario.midStart <-list()
...
for (i in 2:nRealisations){
  ...
  # Tweak the variables laoded from the input file
  list.input$M0 <- M0
  list.input$time.int <- c(500, modelledDuration)
  list.input$T12 <- c(500, modelledDuration)
  ...
  ## Small bit of post processing
  list.output.M6p7Scenario.midStart[[i]] <- append(list.input, list(model.fit = ETAS.model.fit))
  list.output.M6p7Scenario.midStart[[i]]$runtime <- sum(list.output.M6p7Scenario.midStart[[i]]$model.fit$bru_timings$Time)
  list.output.M6p7Scenario.midStart[[i]]$nEvents <- length(list.output.M6p7Scenario.midStart[[i]]$catalog[,1])
}
```
...
post.list <- get_posterior_param(input.list = list.output.M6p7Scenario.midStart[[2]])
post.df <-  post.list[[1]]
post.df$id <-2
 
for(i in 3:nRealisations){
 post.list <- get_posterior_param(input.list = list.output.M6p7Scenario.midStart[[i]])
 post.df.tmp <-  post.list[[1]]
 post.df.tmp$id <-i
...
ggplot(post.df, aes(x=x,y=y, group=id, color=factor(id), lty=factor(id))) + 
  ...
  geom_vline(aes(xintercept = value ), data=trueParas, color='black', linetype=2, label="True value") + 
  labs(color='Initial ETAS Para. Set', linetype='Initial ETAS Para. Set' ) +
  ggtitle(paste("Inversion of the 500 day window starting with a M6.7 event, nEvents =", length(M6p7.ETAS.cat$ts )) ) +
  xlab("ETAS Posteriors") +
  theme(axis.text.x = element_text(angle = 45, hjust=1))+
  theme(legend.position='hidden')
...
```
```{r}
list.output.M6p7Scenario.midStart[[1]][[1]]$catalog
