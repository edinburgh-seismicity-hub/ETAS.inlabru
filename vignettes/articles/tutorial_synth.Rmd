...
The function `generate.temporal.ETAS.synthetic()` can be used to generate synthetic catalogues from a temporal ETAS model with fixed parameters spanning a given interval of time. The `generate.temporal.ETAS.synthetic()` takes as input 
...
synth.cat.list <- generate.temporal.ETAS.synthetic(theta = true.param,
                                                   beta.p = beta.p,
                                                   M0 = M0,
                                                   T1 = T1,
                                                   T2 = T2,
...
link.f <- list(mu = \(x) gamma_t(x, 0.3, 0.6),
               K = \(x) unif_t(x, 0, 10),
...
inv.link.f <- list(mu = \(x) inv_gamma_t(x, 0.3, 0.6),
...
th.init <- list(th.mu = inv.link.f$mu(0.5),
...
* `inla.mode`: type of INLA algorithm to be used. Always set it to `experimental`.
...
```{r, message=FALSE}
synth.fit <- Temporal.ETAS(total.data = synth.cat.df,
                            M0 = M0,
                            T1 = T1,
                            T2 = T2,
...
input_list <- list(model.fit = synth.fit,
                   link.functions = link.f)
...
```{r}
post.samp <- post_sampling(input.list = input_list, 
                           n.samp = 1000, 
                           max.batch = 1000, 
                           ncore = 1)
head(post.samp)
...
trig.plot <- triggering_fun_plot(input.list = input_list, 
                                 post.samp = post.samp,
...
Ht.imposed <- data.frame(ts = mean(c(T1,T2)),
                         magnitudes = 6)
# generate second catalogue
set.seed(1)
synth.cat.list.2 <- generate.temporal.ETAS.synthetic(theta = true.param,
                                                   beta.p = beta.p,
                                                   M0 = M0,
                                                   T1 = T1,
                                                   T2 = T2,
...
```{r}
rbind(first = c(N = nrow(synth.cat.df), 
                N.bkg = sum(synth.cat.df$gen == 1), 
                N.after = sum(synth.cat.df$gen > 1)), 
      second = c(N = nrow(synth.cat.df.2), 
...
synth.fit.2 <- Temporal.ETAS(total.data = synth.cat.df.2,
                             M0 = M0,
                             T1 = T1,
                             T2 = T2,
                             link.functions = link.f,
...
```{r}
input_list.2 <- list(model.fit = synth.fit.2,
                     link.functions = link.f)
```
