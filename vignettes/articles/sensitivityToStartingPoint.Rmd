...
samp.etas.list <- generate_temporal_ETAS_synthetic(theta = theta_etas, 
...
```{r}
plot_omori <- list()
plot_omori[[1]] <- omori_plot_posterior(list.output.quietScenario[[2]], n.samp = 100) +
...
plot_omori[[2]] <- omori_plot_posterior(list.output.M6p7Scenario[[2]], n.samp = 100) +
...
```{r message=FALSE}
post_pairs_plot(list.output.quietScenario[[2]], n.samp=1000)$pair.plot
```
```{r}
post_pairs_plot(list.output.M6p7Scenario[[3]], n.samp=1000)$pair_plot
```
...
```
```{r warning=FALSE}
bru_convergence_plot(list.output.quietScenario[[3]]$model.fit)
```
```{r warning=FALSE}
bru_convergence_plot(list.output.M6p7Scenario[[2]]$model.fit)
```
```{r warning=FALSE}
bru_convergence_plot(list.output.M6p7Scenario[[3]]$model.fit)
```
