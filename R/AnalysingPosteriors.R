...
#'
#' @param input.list input.list structured input `list` with at least two
#'   elements:
#' \itemize{
...
#' \itemize{
#' \item `post.df` : `data.frame` with the posterior distributions of the
#' parameters with columns `x` (value of the parameter), `y` (value of the
#' posterior), `param` (parameter name)
#' \item `post.plot` : `ggplot` object showing the posterior distribution of
#' each parameter}
#' @export
...
#' @param max.batch Maximum number of posterior samples to be generated
#'   simultaneously. Default is 1000.
...
#'
#' @return `data.frame` of posterior samples with `nrow = n.samp` and columns
...
post_sampling <- function(input.list, n.samp, max.batch = 1000, ncore = NULL) {
  ...
  if (n.samp > max.batch) {
    ...
    if (n.samp - max.batch * n.batch == 0) {
      ...
      n.samp.per.batch <- c(
        rep(max.batch, n.batch),
        n.samp - max.batch * n.batch
      )
    }
...
#' @param post.samp `data.frame` with columns mu, K, alpha, c, p and rows
#'   corresponding to different posterior samples. When `NULL` the function
#'   samples the joint posterior distribution `n.samp` times. The default is
#'   `NULL`.
#' @param max.batch parameter of [post_sampling] function to be used in case
#'   `post.samp = NULL`
#' @return `list`: with elements
#' \itemize{
#' \item `post.samp.df`:`data.frame` of posterior samples with `nrow = n.samp`
#' and columns `mu, K, alpha, c, p` corresponding to ETAS parameters. If
#' `post.samp` is not `NULL` it returns `post.samp`
#' \item `pair.plot`: `ggplot` object reporting the pair plot between parameters
#' samples. It is obtained using the `ggpairs` function of the `Ggally` library
#' }
#' @export
post_pairs_plot <- function(input.list = NULL, n.samp = NULL,
  ...
  if (is.null(input.list) && is.null(post.samp)) {
    stop(paste0(
      "input.list and post.samp are missing,",
  ...
  if (is.null(post.samp)) {
    if (is.null(n.samp)) {
      stop(paste0(
        "n.samp is missing, please provide a",
        " number of samples from the posterior"
      ))
    }
    post.samp <- post_sampling(input.list,
      n.samp = n.samp,
      max.batch = max.batch
    )
  }
...
#'
#' @description Calculate the number of events in a time interval T1 to T2 given
#'   imposed events and ETAS
#'
...
#' @param th.alpha ETAS triggering parameter `alpha` on the internal parameter
#'   scale
...
#' @param Ht History of the process, or set of known events in the interval. It
#'   must be a `data.frame` with columns `ts` (time) and `magnitudes`
#'   (magnitudes).
#' @param link.functions `list` of functions to transform the parameters from
#'   the internal scale to the ETAS scale
#'
#' @return Integral of the ETAS conditional intensity between `T1` and `T2` with
...
lambda_N <- function(th.mu, th.K, th.alpha, th.c, th.p, T1, T2, M0, Ht,
  ...
  (theta_etas$mu * (T2 - T1) + sum(exp(log_Lambda_h(
    theta = theta_etas,
    th = Ht$ts,
    mh = Ht$magnitudes,
    M0 = M0,
...
##              (iii) Plot with shaded stack of all distributions of N and a
##              normal about the mean model

#' Plot the posterior distribution of the expected number of events
#'
#' @param input.list Which has combined the input file (for link functions) and
#'   bru output (for marginals)
#' @param domain.extension Percentage of posterior quantiles to extend the
#'   domain specified as `scalar`. Default is set to 0.10.
#'
#' @return A `list` of three objects:
#' \itemize{
#' \item `post.df`: `data.frame` containing posterior informations on the
#' posterior distribution of the number of events
#' \item `post.plot` : `ggplot` object showing the posterior distribution of the
#' expected number of events
#' \item `post.plot.shaded` : `ggplot` object showing the posterior distribution
#' of the expected number of events, shaded region corresponds to the 0.025 and
#' 0.975 quantiles of the distribution of the distribution of the number of
#' events.}
#' @export
get_posterior_N <- function(input.list, domain.extension = 0.10) {
  lambda.N.post <- predict(
    # model fit
    input.list$model.fit,
    # data (empty because the history of the process is passed to the function
    # below directly)
    data.frame(),
    ~ lambda_N(
  ...
  N.seq <- seq.int(
    floor(lambda.N.post$q0.025 - lambda.N.post$q0.025 * domain.extension),
    ceiling(lambda.N.post$q0.975 + lambda.N.post$q0.975 * domain.extension),
    by = 1L
  )
  N.post.df <- predict(
  ...
  N.post.plot <-
    ggplot2::ggplot(N.post.df, ggplot2::aes(x = .data$N, y = .data$mean)) +
    ggplot2::geom_line() +
    ggplot2::geom_vline(
      xintercept = nrow(input.list$catalog.bru),
      linetype = 2
    ) +
    ggplot2::ylab("pdf")
