...
#' @param coef.t TimeBinning parameter: \eqn{\delta > 0} determines the relative
#'   length of subsequent intervals, `scalar`.
#' @param delta.t TimeBinning parameter: \eqn{\Delta > 0} determines the length
#'   of the intervals, `scalar`.
#' @param N.exp. TimeBinning parameter: \eqn{n_{max} >0} determines the maximum
#'   number of intervals, `scalar`
#'
#' @details The grid is calculated as follows \deqn{t, t + \Delta, t + \Delta(1
#'   + \delta), t + \Delta(1 + \delta)^2,...., T} where \eqn{t} is the
#'   `start.grid` argument, \eqn{T} is the `end.grid` argument, and
#'   \eqn{n_{max}} is the maximum value of the exponent
...
#'
#' @return A `data.frame` with as many rows as the number of bins and fixed
#' number of columns.
#' The columns are
#' \itemize{
...
time_grid <- function(data.point, coef.t, delta.t, N.exp.,
  ...
  # create data.frame with t.start (bin starting time), t.end (bin end time) and
  # t.bin.name (unique name for each bin)
  ...
  if (sum(time.bins$t.start <= T1. & time.bins$t.end >= T1.) > 0) {
    time.bins$t.ref_layer[time.bins$t.start <= T1. & time.bins$t.end >= T1.] <-
      paste0("between-", idx.p)
...
#' @description Function to compute the integral of Omori's law efficiently
#'
#' @param param. ETAS parameters vector (\eqn{\mu, K, \alpha, c, p}), only
#'   \eqn{c, p} are used.
#' @param list.input_ `list` containing information to calculate the integrals
#'   efficiently. The list is created inside the `Temporal.ETAS` function
#' Two elements are used
#' \itemize{
#' \item `time.sel` selection of rows of the output of `time_grid` with unique
#' `t.ref_layer` value, `data.frame`.
#' \item `Imapping` mapper between the unique names provided in `time.sel` and
#' original rows of the output of `time_grid()`, `vector`.
#' }
#' @return `vector` with same length as `list.input_$Imapping` with the integral
#'   of Omori's law for each bin.
